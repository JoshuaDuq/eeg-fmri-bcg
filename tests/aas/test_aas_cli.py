...
import bcg_correction.cli as cli_module
...
def test_detect_cardiac_command_executes_independent_marker_pipeline(
    ...
    assert main(["detect-cardiac", "--config", str(config_path)]) == 0
...
def test_benchmark_bcg_command_loads_configuration(
    ...
    monkeypatch.setattr(cli_module, "run_bcg_benchmark", fake_run)

    assert main(["benchmark-bcg", "--config", str(config_path)]) == 0
