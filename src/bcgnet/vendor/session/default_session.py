...
class Session:
    ...
    def _absolute_path_to_data(d_data, str_sub, vec_idx_run, input_file_naming_format):
        ...
        for idx_run in vec_idx_run:
            stem = d_data / str_sub / input_file_naming_format.format(str_sub, idx_run)
            for suffix in ('.set', '.vhdr', '.fif'):
                candidate = Path(str(stem) + suffix)
                if candidate.exists():
                    vec_abs_path.append(candidate)
                    break
            else:
                raise FileNotFoundError(f"No dataset found for {stem} (.set/.vhdr/.fif)")
    ...
    def _absolute_path_to_eval(d_eval, str_sub, vec_idx_run, eval_file_naming_format):
        ...
        for idx_run in vec_idx_run:
            stem = d_eval / str_sub / eval_file_naming_format.format(str_sub, idx_run)
            for suffix in ('.set', '.vhdr', '.fif'):
                candidate = Path(str(stem) + suffix)
                if candidate.exists():
                    vec_abs_path.append(candidate)
                ...
                raise FileNotFoundError(f"No eval dataset found for {stem} (.set/.vhdr/.fif)")
