...
class RNNModel(NNModel):
    def __init__(self, n_input=1, n_output=63, lr=1e-3, opt_type='adam', **kwargs):
        ...
        if opt_type.lower() == 'adam':
            try:
                self.optimizer = optimizers.Adam(learning_rate=lr, **opt_kwargs)
            except TypeError:
                self.optimizer = optimizers.Adam(lr=lr, **opt_kwargs)

        elif opt_type.lower() == 'rmsprop':
            try:
                self.optimizer = optimizers.RMSprop(learning_rate=lr, **opt_kwargs)
            except TypeError:
                self.optimizer = optimizers.RMSprop(lr=lr, **opt_kwargs)

        elif opt_type.lower() == 'sgd':
            try:
                self.optimizer = optimizers.SGD(learning_rate=lr, **opt_kwargs)
            except TypeError:
                self.optimizer = optimizers.SGD(lr=lr, **opt_kwargs)
    ...
    def _model_tf_v2(n_input, n_output):
        ...
        try:
            ...
            _ = tf.compat.v1.Session(config=session_config)
        except Exception:
            pass
        ...
        try:
            ...
            pass
        ecg_input = Input(shape=(None, 1), dtype='float32', name='ecg_input')
        ...
        def _gru(units):
            try:
                return GRU(units, implementation=2, **gru_kwargs)
            except TypeError:
                return GRU(units, **gru_kwargs)

        x = Bidirectional(_gru(16))(ecg_input)
        x = Bidirectional(_gru(16))(x)
        x = Dense(8, activation='relu')(x)
        x = Dropout(0.327)(x)
        x = Bidirectional(_gru(16))(x)
        x = Bidirectional(_gru(64))(x)
    ...
    def save_model_weights(self, p_weights, f_weights, overwrite=True, format='tf'):
        ...
        path = str(p_weights / f_weights)
        if int(tf.__version__[0]) > 1:
            keras_path = path if path.endswith(('.weights.h5', '.h5', '.keras')) else path + '.weights.h5'
            try:
                self.model.save_weights(filepath=path, overwrite=overwrite, save_format=format)
            except TypeError:
                self.model.save_weights(filepath=keras_path, overwrite=overwrite)
        else:
            self.model.save_weights(filepath=path, overwrite=overwrite, save_format='h5')
