...
class Dataset:
    ...
    def _load_dataset(d_run, **kwargs):
        ...
        if d_run.stat().st_size <= 0:
            raise RuntimeError("Invalid dataset")

        suffix = d_run.suffix.lower()
        if suffix == '.set' and 'all' not in d_run.stem:
            run_dataset = mne.io.read_raw_eeglab(str(d_run), preload=True, **kwargs)
        elif suffix == '.vhdr':
            run_dataset = mne.io.read_raw_brainvision(str(d_run), preload=True, **kwargs)
        elif suffix in {'.fif', '.fiff'}:
            run_dataset = mne.io.read_raw_fif(str(d_run), preload=True, **kwargs)
        else:
            raise RuntimeError("Invalid dataset")

        rename = {}
        for name in run_dataset.ch_names:
            if name.lower() == 'fpz' and name != 'Fpz':
                rename[name] = 'Fpz'
            elif name.lower() == 'cpz' and name != 'CPz':
                rename[name] = 'CPz'
        if rename:
            run_dataset.rename_channels(rename)
    ...
    def _perform_epoching(raw_dataset, len_epoch, mad_threshold, str_ecg_channel):
        ...
        orig_epoched_dataset = mne.Epochs(
            raw_dataset, constructed_events, tmin=0, tmax=tmax,
        ...
        if len(vec_idx_good_epochs) != orig_epoched_dataset.get_data().shape[0]:
            ...
            epoched_dataset = mne.EpochsArray(good_epocehd_data, orig_epoched_dataset.info, verbose=False)
    ...
    def _extract_good_epochs(raw_dataset, len_epoch, vec_idx_good_epochs):
        ...
        epoched_dataset = mne.EpochsArray(epoched_data, orig_epoched_dataset.info, verbose=False)
    ...
    def _plot_random_epoch(epoched_raw_dataset_set, epoched_eval_dataset_set, epoched_cleaned_dataset_set,
        ...
        if epoched_eval_dataset_set is not None:
            ...
            eval_t = np.arange(eval_eeg_data_set.shape[1]) / epoched_eval_dataset_set.info['sfreq']
            if eval_eeg_data_set.shape[1] == t.shape[0]:
                plt.plot(t, eval_eeg_data_set[idx_epoch, :], 'C2', label=str_eval)
            else:
                plt.plot(eval_t, eval_eeg_data_set[idx_epoch, :], 'C2', label=str_eval)
